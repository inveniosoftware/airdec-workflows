...
async def create_workflow(
    ...
    workflow = Workflow(status="PROCESSING", url=body.url, user_id="123")
    try:
...
async def read_workflow(
    ...
    try:
        workflow = (
            session.query(Workflow).filter(Workflow.public_id == workflow_id).one()
        )
        ...
        with Session(request.app.state.db_engine) as session:
            try:
                workflow = (
                    ...
                    .filter(Workflow.public_id == workflow_id)
                    .one()
                )
